...
public class UserRepositorySJdbc implements UserRepository {
        ...
        try {
            return Optional.ofNullable(
                    authTemplate.query(
                            "SELECT * " +
                                    "FROM \"user\" u " +
                                    "JOIN \"authority\"  ON u.id = \"authority\".user_id " +
                                    "where u.id = ?",
    ...
    public UserAuthEntity updateUserInAuth(UserAuthEntity user) {
        return authTxt.execute(new TransactionCallback<UserAuthEntity>() {
            @Override
            public UserAuthEntity doInTransaction(TransactionStatus status) {
                authTemplate.update(new PreparedStatementCreator() {
                    @Override
    ...
    public UserEntity updateUserInData(UserEntity user) {
        udTemplate.update(con -> {
            PreparedStatement ps = con.prepareStatement("UPDATE \"user\" SET username = ?, currency = ? WHERE id = ?");
            ps.setString(1, user.getUsername());
            ps.setString(2, user.getCurrency().name());
